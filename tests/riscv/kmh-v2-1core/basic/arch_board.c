...
void arch_board_fdt_fixup(void *fdt_addr)
{
	...
	// 设置 /cpus 的属性, 写入 timebase-frequency
	...
	for (i = 0; i < vcpu_count; i++) {
		...
		// mmu-type 设置为 riscv,sv39
		ret = fdt_setprop_string(fdt_addr, cpu_offset,
					 "mmu-type", "riscv,sv39");	/* 注: kmh 支持的是 riscv 的版本 */
		if (ret < 0) {
			basic_printf("Failed to set %s property of /cpus/%s "
				     "DT node\n", "mmu-type", name);
			return;
		...
		// riscv,isa 调用 sbi_xvisor_isa_string 获取
		...
		// 并设置 compatible, interrupt-controller, interrupt-cells, phandle, linux, phanle
